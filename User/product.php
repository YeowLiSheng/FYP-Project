...
let filters = { price: 'all', color: 'all', tag: 'all' }; // Set default filters to 'all'

		// Function to update product display based on filters
		...
		// Handle filter clicks and toggling selection
		$(document).on('click', '.filter-link', function(event) {
			...
			if (filters[filterType] === filterValue) {
				filters[filterType] = 'all'; // Reset to 'all' if already selected
				$(this).removeClass('selected'); // Remove blue highlight
			} else {
				filters[filterType] = filterValue; // Set new filter value
